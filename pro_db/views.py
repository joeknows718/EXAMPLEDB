...
class DashBoardView(LoginRequiredMixin, ApprovedMixin, View):
	...
	def get(self, request, *args, **kwargs): 
		...
		context['states_upcoming_general'] = State.objects.all().annotate().filter(district__general_election_date__gt=date.today()).order_by('state_name').distinct('state_name')
		context['states_upcoming_primary'] = State.objects.all().annotate().filter(district__primary_election_date__gt=date.today()).order_by('state_name').distinct('state_name')
		context['states_filing'] = State.objects.all().annotate().filter(district__next_filing_date__gt=date.today()).order_by('state_name').distinct('state_name')
...
class CandidateReportGen(LoginRequiredMixin,
	...
	def form_valid(self, form):
		...
		if gender_param is None or len(gender_param) < 1:
			pass
		else:
		...
		if race_param is None or len(gender_param) < 1:
			pass
		else:
			queryset = queryset.filter(race=race_param)
